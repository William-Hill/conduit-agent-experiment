...
`changedFiles` comes from a new helper `collectChangedFiles(ctx, repoDir) ([]string, error)` that runs `git add -N .` (to stage untracked files) and parses `git status --porcelain` into a slice of paths. Refactor: the old `collectDiff` used to do all three jobs at once (`git add -N .`, `git diff HEAD`, and `git status --porcelain`). After the refactor, `Review()` calls `collectChangedFiles` first — so the changed-file set is available for lint filtering — and then calls `collectDiff` separately for the `git diff HEAD` output the semantic reviewer needs. `collectDiff` no longer touches `git status --porcelain`; the two helpers are independent and `Review()` is the only place that sequences them. The porcelain parser reads the path as `line[3:]`, handles rename arrows (`ORIG -> DEST`), and strips the C-string double-quotes git adds around paths containing spaces or other special characters — `strings.Fields` cannot be used because it would shatter filenames with spaces. A parsed lint error matches a changed file when its `File`, after stripping a leading `./` and any `repoDir` prefix, equals any entry in `changedFiles`.
