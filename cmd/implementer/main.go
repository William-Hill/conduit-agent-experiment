...
func main() {
	...
	// 4. Run implementer agent
	log.Printf("Running implementer agent (max %d iterations)...", maxIter)
	result, err := implementer.RunAgent(ctx, apiKey, repoDir, fullIssue.Title, fullIssue.Body, maxIter)
	if err != nil {
	...
	// 6. Create branch, commit, push, draft PR
	...
	prURL, err := adapter.CreateDraftPR(ctx, github.DraftPRInput{
		...
		Body:  fmt.Sprintf("Fixes #%d\n\n## Agent Summary\n\n%s\n\n---\nGenerated by conduit-agent-experiment implementer (%d iterations).", issue.Number, result.Summary, result.Iterations),
		Head:  branch,
		Base:  "main",
	})
	if err != nil {
