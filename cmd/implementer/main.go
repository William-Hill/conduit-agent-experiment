...
func main() {
	...
	if artifactDir != "" {
		writeRunArtifacts(artifactDir, issue, result, backend.Name(), result.ModelName, plan, countHallucinationsInRepo(ctx, repoDir))
	}
	...
	modelDisplay := result.ModelName
	if modelDisplay == "" {
		modelDisplay = modelName // fall back to env-var default if backend didn't populate
	}
	if modelDisplay == "" {
